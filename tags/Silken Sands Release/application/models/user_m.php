	...
	function login($data) {
		...
		if ($query->result_array () == null) {
			...
			foreach ( $query->result_array () as $row ) {
				$user_info = array ('username' => $row ['username'], 'designation' => $row ['designation'], 'order' => $row ['order_manage'], 'menu' => $row ['menu_manage'], 'database' => $row ['database_manage'], 'report' => $row ['report_manage'], 'user' => $row ['user_manage'] );
			}
			return $user_info;
		}
